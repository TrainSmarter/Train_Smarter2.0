...
/**
 * Tests for /api/teams/invite-token route.
 *
...
describe("/api/teams/invite-token POST: Auth guard", () => {
  ...
  it("should read teamInviteToken from httpOnly cookie", () => {
    expect(routeSource).toContain('cookies.get("teamInviteToken")');
  });
...
describe("/api/teams/invite-token GET: Token validation", () => {
  ...
  it("should redirect to dashboard page (not register)", () => {
    expect(routeSource).toContain('new URL("/dashboard"');
...
describe("/api/teams/invite-token: Security", () => {
  it("should use httpOnly to prevent XSS exfiltration", () => {
    expect(routeSource).toContain("httpOnly: true");
    expect(routeSource).toContain("prevents XSS exfiltration");
  });

...
const SUBJECTS: Record<Locale, (trainerName: string) => string> = {
  de: (name) => `${name} möchte sich mit dir auf Train Smarter verbinden`,
  en: (name) => `${name} wants to connect with you on Train Smarter`,
};
...
<html lang="de">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Verbindungsanfrage von {{trainerName}}</title></head>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f8fafc;padding:40px 0;"><tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(15,23,42,0.08);">
      ...
      <tr><td style="padding:40px;">
        <h2 style="margin:0 0 16px;color:#0f172a;font-size:20px;font-weight:600;">Neue Verbindungsanfrage</h2>
        <p style="margin:0 0 24px;color:#475569;font-size:15px;line-height:1.6;">
          <strong>{{trainerName}}</strong> m&ouml;chte sich mit dir auf Train Smarter verbinden. Melde dich an, um die Anfrage anzunehmen oder abzulehnen.
        </p>
        ...
        <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;"><tr>
          <td style="background-color:#0D9488;border-radius:8px;">
            <a href="{{dashboardLink}}" target="_blank" style="display:inline-block;padding:12px 32px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;">Anfrage ansehen</a>
          </td>
        </tr></table>
        <p style="margin:0;color:#94a3b8;font-size:13px;line-height:1.5;">Falls du diese Anfrage nicht erwartet hast, kannst du diese E-Mail ignorieren.</p>
      </td></tr>
...
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Connection Request from {{trainerName}}</title></head>
<body style="margin:0;padding:0;background-color:#f8fafc;font-family:'Inter',system-ui,-apple-system,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f8fafc;padding:40px 0;"><tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 3px rgba(15,23,42,0.08);">
      <tr><td style="background:linear-gradient(135deg,#0D9488,#0F766E);padding:32px 40px;text-align:center;">
      ...
      <tr><td style="padding:40px;">
        <h2 style="margin:0 0 16px;color:#0f172a;font-size:20px;font-weight:600;">New Connection Request</h2>
        <p style="margin:0 0 24px;color:#475569;font-size:15px;line-height:1.6;">
          <strong>{{trainerName}}</strong> wants to connect with you on Train Smarter. Sign in to accept or decline the request.
        </p>
        {{personalMessageBlock}}
        <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;"><tr>
          <td style="background-color:#0D9488;border-radius:8px;">
            <a href="{{dashboardLink}}" target="_blank" style="display:inline-block;padding:12px 32px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;">View Request</a>
          </td>
        </tr></table>
        <p style="margin:0;color:#94a3b8;font-size:13px;line-height:1.5;">If you were not expecting this request, you can safely ignore this email.</p>
      </td></tr>
      <tr><td style="padding:24px 40px;background-color:#f8fafc;border-top:1px solid #e2e8f0;text-align:center;">
        <p style="margin:0 0 8px;color:#94a3b8;font-size:12px;">&copy; Train Smarter &mdash; www.train-smarter.at</p>
...
function renderTemplate(locale: Locale, payload: ConnectionRequestPayload): string {
  ...
  html = html
    ...
    .replace(/\{\{dashboardLink\}\}/g, payload.dashboardLink);
  ...
  try {
    await client.send({
      ...
      headers: {
        ...
        "Feedback-ID": "connection-request:train-smarter:transactional:train-smarter.at",
...
function validatePayload(
  body: unknown
): { valid: true; data: ConnectionRequestPayload } | { valid: false; error: string } {
  ...
  if (typeof b.dashboardLink !== "string") {
    return { valid: false, error: "Invalid dashboardLink" };
  }
  try {
    const url = new URL(b.dashboardLink);
    if (!["http:", "https:"].includes(url.protocol)) {
      return { valid: false, error: "Invalid dashboardLink protocol" };
    }
  } catch {
    return { valid: false, error: "Invalid dashboardLink" };
  }
  ...
  return {
    ...
    data: {
      ...
      dashboardLink: b.dashboardLink as string,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Only trainers can send connection request emails
    ...
    console.log(
      `Connection request email sent: locale=${payload.locale}, to_hash=${emailHash}, trainer=${payload.trainerName}`
    );
    ...
    console.error("send-connection-request-email error:", error);
    return new Response(
      JSON.stringify({
